...
const AnimatedCTASection = () => {
    const lineWrapperTops = ['top-[30%]', 'top-[50%]', 'top-[70%]', 'top-[90%]'];
    ...
    return (
        <>
            ...
            <section className="relative min-h-screen flex items-center justify-center bg-black text-white font-sans overflow-hidden p-8 sm:p-16">
                {/* Grid Background */}
                <div
                    className="absolute inset-0 w-full h-full bg-[linear-gradient(rgba(255,149,0,0.07)_1px,transparent_1px),linear-gradient(90deg,rgba(255,149,0,0.07)_1px,transparent_1px)] bg-[length:50px_50px] anim-grid-move z-0"
                />
                    ...
                    {lineWrapperTops.map((topClass, index) => (
                        <div key={index} className={`absolute w-full h-[100px] ${topClass}`}>
                            <div className="w-full h-0.5 relative overflow-hidden">
                                <div
                                    className={`absolute top-0 w-full h-full anim-line-move ${index % 2 !== 0 ? 'anim-reverse-delay' : ''
                                        }`}
                                    style={{
                ...
                <div className="hidden md:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[100px] z-[5]">
                    <svg
                        className="absolute top-1/2 -translate-y-1/2 left-[-150px] w-[120px] h-[60px] anim-corner-line"
                        viewBox="0 0 120 60"
                        stroke="#ff9500"
                        strokeWidth="2"
                        fill="none"
                    ...
                    <svg
                        className="absolute top-1/2 -translate-y-1/2 right-[-150px] w-[120px] h-[60px] transform scale-x-[-1] anim-corner-line"
...
const GlobalStylesAndKeyframes = () => (
    ...
    .anim-corner-line {
      animation: cornerLineAnimation 6s linear infinite;
    }
    .anim-reverse-delay {
